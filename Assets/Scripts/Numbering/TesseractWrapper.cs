...
public class TesseractWrapper
{
    ...
    private const float MinimumConfidence = 95;
    ...
    private List<Box> _UsedBoxes = new List<Box>();
    private List<Rect> _UsedRects = new List<Rect>();
    ...
    public string Recognize(Texture2D texture)
    {
        ...
        _UsedBoxes = new List<Box>();
        _UsedRects = new List<Rect>();
        ...
        for (int index = 0; index < boxes.Length; index++)
        {
            if (confidence[index] >= MinimumConfidence)
            {
                ...
                Box box = boxes[index];
                Rect rect = new Rect(box.x, _highlightedTexture.height - box.y - box.h, box.w, box.h);
                _UsedBoxes.Add(box);
                _UsedRects.Add(rect);
